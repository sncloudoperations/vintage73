        ...
        <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="card p-6 border-l-4 border-blue-500">
                    <p className="text-[10px] font-extrabold uppercase text-slate-400 tracking-widest leading-none mb-2">Total Referrals</p>
                    <h3 className="text-3xl font-extrabold text-slate-800">{stats.total}</h3>
                </div>
                <div className="card p-6 border-l-4 border-emerald-500">
                    <p className="text-[10px] font-medium uppercase text-slate-400 tracking-widest leading-none mb-2">Won Leas</p>
                    <h3 className="text-3xl font-bold text-emerald-600">{stats.won}</h3>
                </div>
                <div className="card p-6 border-l-4 border-amber-500">
                    <p className="text-[10px] font-medium uppercase text-slate-400 tracking-widest leading-none mb-2">Pending Payout</p>
                    <h3 className="text-3xl font-bold text-amber-600">₹{stats.pendingCommission.toLocaleString()}</h3>
                </div>
                <div className="card p-6 border-l-4 border-purple-500 bg-purple-50/10">
                    <p className="text-[10px] font-extrabold uppercase text-purple-400 tracking-widest leading-none mb-2">Earned Total</p>
                    <h3 className="text-3xl font-bold text-purple-600">₹{stats.paidCommission.toLocaleString()}</h3>
                </div>
            </div>
            ...
            <div className="table-container shadow-xl shadow-slate-200/50 border border-slate-100 rounded-3xl overflow-hidden">
                <table className="table-modern">
                    <thead className="bg-slate-50/50">
                        <tr>
                            <th className="text-[10px] font-medium uppercase tracking-widest py-5">Customer</th>
                            <th className="text-[10px] font-medium uppercase tracking-widest py-5">Product</th>
                            <th className="text-[10px] font-medium uppercase tracking-widest py-5 text-right">Negotiation Amt</th>
                            <th className="text-[10px] font-medium uppercase tracking-widest py-5 text-center">Comm. %</th>
                            <th className="text-[10px] font-medium uppercase tracking-widest py-5 text-right text-purple-600">Earnings</th>
                            <th className="text-[10px] font-medium uppercase tracking-widest py-5">Lead Status</th>
                            <th className="text-[10px] font-medium uppercase tracking-widest py-5">Payment Status</th>
                            <th className="text-right py-5"></th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50 bg-white">
                        {loading ? (
                            <tr><td colSpan="8" className="p-20 text-center text-slate-400 font-medium animate-pulse uppercase tracking-widest letter-spacing-2">Fetching your earnings...</td></tr>
                        ) : filteredLeads.length === 0 ? (
                            <tr><td colSpan="8" className="p-20 text-center text-slate-400 font-medium italic border-dashed border-2 border-slate-50 rounded-3xl m-4 px-20">No referral leads found.</td></tr>
                            ...
                            <tr key={lead.id} className="hover:bg-slate-50/50 transition-all cursor-pointer group" onClick={() => router.push(`/crm/leads/${lead.id}`)}>
                                <td className="py-4">
                                    ...
                                    <p className="text-[9px] text-slate-400 mt-1 uppercase font-medium tracking-tighter">#{lead.id}</p>
                                </td>
                                <td>
                                ...
                                <td>
                                    <span className={`px-3 py-1 rounded-full text-[9px] font-medium uppercase tracking-widest border transition-all ${
                                    ...
                                    <div className="flex items-center gap-2">
                                        {lead.commissionPaid ? (
                                            <>
                                                ...
                                                <span className="text-[10px] font-medium text-emerald-600 uppercase tracking-widest">Paid</span>
                                            </>
                                        ) : (
                                            <>
                                                <div className="w-2 h-2 rounded-full bg-amber-400" />
                                                <span className="text-[10px] font-medium text-amber-500 uppercase tracking-widest">Unpaid</span>
                                            </>
                                        )}
                                    </div>
                                </td>
