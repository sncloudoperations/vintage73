        ...
        <div className="p-8 h-[calc(100vh-64px)] overflow-hidden flex flex-col bg-slate-50/50">
             <div className="flex justify-between items-center mb-8 flex-shrink-0">
                ...
                <button 
                    ...
                    className="flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-2xl font-bold hover:bg-slate-800 transition shadow-xl shadow-slate-900/10 active:scale-95"
                >
                                ...
                                <div key={stageKey} className="w-80 flex flex-col h-full rounded-3xl bg-slate-100/50 border border-slate-200/60 flex-shrink-0 backdrop-blur-sm">
                                    ...
                                    <div className="p-4 rounded-t-3xl bg-white/40 border-b border-slate-200/50 backdrop-blur-md sticky top-0 z-10">
                                        ...
                                        <div className="flex justify-between items-end">
                                            <div>
                                                <h3 className="font-black text-sm uppercase tracking-wider text-slate-700">{config.label}</h3>
                                                <p className="text-xs text-slate-400 font-semibold mt-0.5">{stageDeals.length} Deals</p>
                                            </div>
                                            <div className="text-sm font-bold text-slate-800 bg-white px-2 py-1 rounded-lg border border-slate-100 shadow-sm">
                                                        ...
                                                        {(provided, snapshot) => (
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex justify-between items-start mb-2">
                                                                    <div className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${config.color}`}>
                                                                        {config.label}
                                                                    </div>
                                                                    <FiMoreHorizontal className="text-slate-300 group-hover:text-slate-500 transition-colors" />
                                                                </div>

                                                                <h4 className="font-bold text-slate-800 mb-1 leading-snug">{deal.title}</h4>
                                                                ...
                                                                <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                                                                    {deal.assignedUser ? (
                                                                        <div className="flex items-center gap-1.5 ring-2 ring-white rounded-full bg-slate-50 pr-2">
                                                                            <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 text-[10px] flex items-center justify-center font-bold text-white shadow-sm">
                                                                                {deal.assignedUser.name[0]}
                                                                            </div>
                                                                            <span className="text-[10px] font-bold text-slate-600">{deal.assignedUser.name.split(' ')[0]}</span>
                                                                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                            ...
                            <button onClick={() => setShowModal(false)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 font-bold transition-colors">✕</button>
                        </div>
