...
export default function WonLeads() {
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
            <header>
                <h1 className="text-3xl font-black text-slate-800 tracking-tight">Won Leads (Converted)</h1>
                <p className="text-slate-500 font-medium">Leads that successfully converted into customers and orders</p>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    <div className="col-span-full text-center p-12 text-slate-400 animate-pulse font-black uppercase tracking-widest">Loading Won Leads...</div>
                ) : leads.length === 0 ? (
                    <div className="col-span-full card p-12 text-center text-slate-400 font-bold italic border-dashed border-2 border-slate-100">
                    ...
                    <div key={lead.id} className="card p-6 shadow-sm border-emerald-100 hover:shadow-lg transition-all group">
                        ...
                        <h3 className="font-black text-lg text-slate-800">{lead.name}</h3>
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center p-2 bg-slate-50 rounded-xl">
                                ...
                                <span className="text-xs font-bold text-slate-700">{lead.product?.name || 'Inquiry'}</span>
                            </div>
                            <div className="flex justify-between items-center p-2 bg-emerald-50 rounded-xl">
                                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest leading-none">Deal Value</span>
                                <span className="text-xs font-black text-emerald-700">₹{lead.budget ? parseFloat(lead.budget).toLocaleString() : '1,00,000'}</span>
                            </div>
                        </div>

                        <div className="mt-6 pt-4 border-t border-slate-50 flex items-center gap-2 text-emerald-600">
                            <FiCheckCircle size={14} />
                            <span className="text-[10px] font-black uppercase tracking-widest">Successfully Converted</span>
                        </div>
                    </div>
                ))}
            </div>
