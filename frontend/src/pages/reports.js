    ...
    <div>
      ...
      <div className="card mb-6 border-0 shadow-sm bg-white p-6 rounded-xl">
        <div className="flex flex-wrap gap-6 items-end">
          ...
          <button className="text-sm font-bold text-emerald-600 hover:text-emerald-700 pb-2" onClick={() => { setStartDate(''); setEndDate(''); }}>Reset Filters</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="card border-0 shadow-lg bg-emerald-500 text-white p-5 rounded-2xl relative overflow-hidden flex flex-col justify-center min-h-[120px]">
           <div className="relative z-10">
             <p className="text-emerald-100 text-xs font-bold uppercase tracking-widest mb-1">Total Revenue</p>
      ...
      <div className="mb-8">
        <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
            <FiTrendingUp className="text-emerald-500" /> Daily Payment Method Summary ({startDate || 'Today'})
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {paymentSummary.map(ps => (
                <div key={ps.method} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex justify-between items-center group hover:border-emerald-200 transition-colors">
                    <div>
                    ...
                    <div className="text-right">
                        <div className="text-[10px] text-emerald-500 font-bold leading-none mb-1">+{ps.receipts.toFixed(2)}</div>
              ...
              {filteredSales.map(sale => (
                <tr key={sale.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4 text-center">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${sale.status === 'cancelled' ? 'bg-slate-100 text-slate-500' : (sale.isReturn ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700')}`}>
      ...
      {/* Sale Detail Modal */}
      {selectedSale && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setSelectedSale(null)}>
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="px-8 py-6 border-b flex justify-between items-center bg-slate-50/50">
                      <div>
                          <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-1">Invoice Details</p>
                  ...
                  <div className="px-8 py-6 bg-slate-900 flex justify-between items-center">
                      ...
                      <button 
                        ...
                        className="px-6 py-3 bg-white text-slate-900 rounded-xl font-bold hover:bg-emerald-400 hover:text-slate-900 transition-all flex items-center gap-2 shadow-lg shadow-white/5 active:scale-95"
                      >
