    ...
    <div className="p-4 md:p-8">
      ...
      <div className="flex gap-4 mb-6 border-b border-slate-200 text-sm font-medium">
        <button 
          ...
          className={`pb-2 px-1 border-b-2 transition-colors ${activeTab === 'new' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
        >
          New Transfer
        </button>
        <button 
          onClick={() => setActiveTab('history')}
          className={`pb-2 px-1 border-b-2 transition-colors ${activeTab === 'history' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
        >
          Outgoing History
        </button>
      </div>
      ...
      {activeTab === 'new' ? (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 max-w-4xl">
          ...
          <div className="flex justify-between items-center pt-4 border-t border-slate-100">
            <button onClick={handleAddRow} className="text-emerald-600 font-bold flex items-center gap-2 hover:bg-emerald-50 px-4 py-2 rounded-xl transition-colors">
              <FiPlus /> Add Another Item
            </button>
            <button onClick={handleSubmit} className="bg-emerald-600 text-white font-bold px-8 py-3 rounded-2xl flex items-center gap-2 hover:bg-emerald-700 shadow-lg shadow-emerald-100 transition-all active:scale-95">
                ...
                <tr key={t.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="p-4 text-center">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${
                      ...
                      t.status === 'RECEIVED' ? 'bg-emerald-100 text-emerald-600' : 
                  ...
                  <td className="p-4 text-right pr-6 flex justify-end gap-2">
                    <button 
                      ...
                      className="text-emerald-600 hover:bg-emerald-50 px-2 py-1 rounded-lg text-[10px] font-bold uppercase transition-colors flex items-center gap-1"
                    >
                      <FiPrinter /> Print
                    </button>
                    {t.status === 'PENDING' && (
