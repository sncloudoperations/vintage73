            ...
            <div className="flex flex-col gap-4 mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="flex flex-col gap-1 flex-1">
                        <label className="text-[10px] font-medium text-slate-500 uppercase">Search</label>
                            ...
                            {filteredSales.length === 0 ? (
                                ...
                                filteredSales.map(sale => (
                                    <tr key={sale.id} className="hover:bg-slate-50/50 transition-all text-sm">
                                        <td className="p-4">
                                            <p className="text-slate-800 font-medium text-xs mb-0.5" style={{ color: theme.primaryColor }}>#{sale.invoiceNumber}</p>
                                            <div className="flex items-center gap-1 text-[9px] text-slate-400 font-medium uppercase">
                                                <FiCalendar size={10} /> {new Date(sale.saleDate).toLocaleDateString()}
                                            </div>
                                        </td>
                                        <td className="p-4">
                                            <p className="text-slate-700 font-medium text-xs mb-0.5">{sale.customer?.name || 'Walk-in Profile'}</p>
                                            <p className="text-[9px] text-slate-400 font-medium">{sale.customer?.phone || 'No Phone'}</p>
                                        </td>
                                        <td className="p-4">
                                            <div className="flex flex-col gap-1">
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md text-[9px] font-medium w-fit uppercase">
                                                    {sale.paymentMethod || 'Cash'}
                                                </span>
                                                <span className="text-[9px] text-slate-400 font-medium uppercase ml-0.5 italic">By: {sale.salesman?.name || 'System'}</span>
                                            </div>
                                        </td>
                                        <td className="p-4 text-right font-medium text-slate-900 tabular-nums text-sm">
                                            ₹{parseFloat(sale.totalAmount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                        </td>
                                        <td className="p-4 text-center">
                                            <span className={`px-2 py-1 rounded-lg text-[9px] font-medium uppercase tracking-widest ${sale.status === 'cancelled'
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-sm" onClick={() => setSelectedSale(null)}>
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in duration-200" onClick={e => e.stopPropagation()}>
                        <div className="px-6 py-4 border-b flex justify-between items-center bg-slate-50/50">
                            <div>
                                <p className="text-[9px] font-medium text-primary uppercase tracking-widest mb-0.5">Invoice #</p>
                                <h3 className="text-base font-medium text-slate-900">#{selectedSale.invoiceNumber}</h3>
                            </div>
                        ...
                        <div className="p-0 overflow-y-auto max-h-[50vh]">
                            <div className="px-6 py-4 grid grid-cols-2 gap-8 bg-white">
                                <div className="space-y-1">
                                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest">Customer</p>
                                    <p className="text-sm font-medium text-slate-800">{selectedSale.customer?.name || 'Walk-in'}</p>
                                </div>
                                <div className="space-y-1 text-right">
                                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest">Date</p>
                                    <p className="text-sm font-medium text-slate-800">{new Date(selectedSale.saleDate).toLocaleDateString()}</p>
                                </div>
                            </div>
                            ...
                            {selectedSale.status === 'cancelled' && (
                                <div className="mx-6 mb-4 p-3 bg-rose-50 rounded-xl border border-rose-100 flex items-center gap-3">
                                    ...
                                    <div className="text-[10px] text-rose-700 font-medium">Voided by {selectedSale.cancelledBy} on {new Date(selectedSale.cancelledAt).toLocaleDateString()}</div>
                                </div>
                            )}
                                ...
                                <table className="w-full text-xs">
                                    <thead>
                                        <tr className="text-slate-400 border-b-2 border-slate-50">
                                            <th className="py-2 text-left font-medium uppercase text-[8px] tracking-widest">Product</th>
                                            <th className="py-2 text-center font-medium uppercase text-[8px] tracking-widest">Qty</th>
                                            <th className="py-2 text-right font-extrabold uppercase text-[8px] tracking-widest">Total</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-50">
                                        {selectedSale.items?.map(item => (
                                            <tr key={item.id}>
                                                <td className="py-3">
                                                    <p className="font-medium text-slate-800">{item.product?.name || "Inventory Item"}</p>
                                                    <p className="text-[9px] text-slate-400 font-medium">₹{parseFloat(item.unitPrice).toFixed(2)}</p>
                                                </td>
                                                <td className="py-3 text-center text-slate-900 font-medium">{item.quantity}</td>
                                                <td className="py-3 text-right font-extrabold text-indigo-600">₹{parseFloat(item.total).toLocaleString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div className="px-6 py-4 bg-slate-900 flex justify-between items-center">
                            <div>
                                <p className="text-[8px] font-extrabold text-indigo-300 uppercase tracking-widest">Total Amount</p>
                                <p className="text-xl font-extrabold text-white tabular-nums">₹{parseFloat(selectedSale.totalAmount).toLocaleString()}</p>
                            </div>
                            <button onClick={() => triggerPrint(selectedSale)} className="px-5 py-2.5 bg-white text-slate-900 rounded-xl font-medium text-xs hover:bg-primary hover:text-white transition-all transform active:scale-95 flex items-center gap-2">
                ...
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-xs overflow-hidden">
                        <div className="p-6 text-center">
                            ...
                            <h3 className="text-lg font-medium text-slate-900 mb-1">Cancel Invoice?</h3>
                            <p className="text-slate-500 text-[11px] mb-6">This will reverse stock and accounting entries.</p>
                            <textarea
                                className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 text-xs focus:ring-2 focus:ring-rose-500 focus:outline-none transition-all font-medium text-slate-700 min-h-[80px] mb-4"
                                placeholder="Reason for cancellation..."
                                value={cancelReasonInput}
                                onChange={e => setCancelReasonInput(e.target.value)}
                            ></textarea>
                            <div className="grid grid-cols-2 gap-3">
                                <button onClick={() => setShowCancelModal(false)} className="py-3 bg-slate-100 text-slate-600 rounded-xl font-medium text-xs hover:bg-slate-200">Go Back</button>
                                <button onClick={confirmCancel} disabled={loading} className="py-3 bg-rose-600 text-white rounded-xl font-medium text-xs hover:bg-rose-700 disabled:opacity-50">Cancel Invoice</button>
                            </div>
                        </div>
                    </div>
                </div>
