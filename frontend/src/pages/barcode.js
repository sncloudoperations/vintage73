    ...
    <div className="space-y-6 max-w-[1600px] mx-auto">
      <div className="flex justify-between items-center print:hidden">
        ...
        <div className="flex gap-3">
            ...
            <button className="flex items-center gap-2 px-6 py-2 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-200" onClick={handlePrint} disabled={!selectedProduct}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3 card h-fit print:hidden">
          ...
          <div className="relative mb-4">
             ...
             <input 
               className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20" 
          ...
          <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1 custom-scrollbar">
            {filteredProducts.map(product => (
              <div 
                key={product.id} 
                className={`p-3 rounded-xl cursor-pointer border-2 transition-all ${selectedProduct?.id === product.id ? 'bg-emerald-50 border-emerald-500' : 'bg-white border-slate-100 hover:border-slate-300'}`}
                onClick={() => setSelectedProduct(product)}
              >
                <div className="flex justify-between items-start">
                    <p className={`font-bold text-sm ${selectedProduct?.id === product.id ? 'text-emerald-900' : 'text-slate-700'}`}>{product.name}</p>
                    {selectedProduct?.id === product.id && <FiCheck className="text-emerald-500" />}
                </div>
        ...
        <div className="lg:col-span-6 bg-slate-100/50 rounded-2xl border-2 border-dashed border-slate-200 p-8 flex flex-col items-center justify-center min-h-[500px] print:w-full print:bg-white print:border-0 print:p-0 overflow-auto">
          ...
          <div className="mt-8 px-4 py-2 bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase tracking-widest rounded-full border border-emerald-100 print:hidden animate-pulse">
            Live Preview
          </div>
        </div>
          ...
          <div className="space-y-8">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex bg-white rounded-lg p-1 border border-slate-200">
                            <button 
                                ...
                                className={`px-2 py-1 text-[10px] font-bold rounded ${settings.paperSize === 'Single' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-500'}`}
                            >
                                Single
                            </button>
                            <button 
                                onClick={() => setSettings({...settings, paperSize: 'Grid'})}
                                className={`px-2 py-1 text-[10px] font-bold rounded ${settings.paperSize === 'Grid' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-500'}`}
                            >
                                Multi-up
                            </button>
                        </div>
            ...
            <div>
                ...
                <div className="flex p-1 bg-slate-100 rounded-lg gap-1">
                    <button 
                        className={`flex-1 py-1.5 rounded-md flex justify-center transition-all ${settings.alignment === 'left' ? 'bg-white shadow-sm text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}
                        onClick={() => setSettings({...settings, alignment: 'left'})}
                    >
                        <FiAlignLeft size={16} />
                    </button>
                    <button 
                         className={`flex-1 py-1.5 rounded-md flex justify-center transition-all ${settings.alignment === 'center' ? 'bg-white shadow-sm text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}
                        onClick={() => setSettings({...settings, alignment: 'center'})}
                    >
                        <FiAlignCenter size={16} />
                    </button>
                    <button 
                         className={`flex-1 py-1.5 rounded-md flex justify-center transition-all ${settings.alignment === 'right' ? 'bg-white shadow-sm text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}
            ...
            <div className="pt-4 border-t border-slate-100 space-y-3">
                <div className="flex items-center justify-between">
                    ...
                    <button 
                        ...
                        className={`w-8 h-4 rounded-full transition-colors relative ${settings.showName ? 'bg-emerald-500' : 'bg-slate-300'}`}
                    >
                ...
                <div className="flex items-center justify-between">
                    ...
                    <button 
                        ...
                        className={`w-8 h-4 rounded-full transition-colors relative ${settings.showPrice ? 'bg-emerald-500' : 'bg-slate-300'}`}
                    >
                ...
                <div className="flex items-center justify-between">
                    ...
                    <button 
                        ...
                        className={`w-8 h-4 rounded-full transition-colors relative ${settings.showBarcodeValue ? 'bg-emerald-500' : 'bg-slate-300'}`}
                    >
