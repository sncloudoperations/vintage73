    ...
    <div>
        ...
        {activeTab === 'entry' && (
            <div className="flex items-center gap-3 bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100">
                <div className="text-right">
                    <p className="text-xs text-emerald-600 font-medium uppercase tracking-wider">Estimate Total</p>
                    <p className="text-2xl font-bold text-emerald-700">₹ {grandTotal.toFixed(2)}</p>
                </div>
                <FiTruck className="text-2xl text-emerald-500" />
            </div>
        )}
      </div>
      ...
      <div className="flex gap-4 mb-6 border-b border-slate-200">
        <button 
            ...
            className={`pb-3 px-4 font-bold flex items-center gap-2 transition-colors border-b-2 ${activeTab === 'entry' ? 'border-emerald-500 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
        >
            <FiPlus /> New Entry
        </button>
        <button 
            onClick={() => setActiveTab('history')}
            className={`pb-3 px-4 font-bold flex items-center gap-2 transition-colors border-b-2 ${activeTab === 'history' ? 'border-emerald-500 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
        >
          ...
          <div className="card shadow-lg border-0 bg-white p-6 rounded-xl">
            <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <div className="text-xs text-slate-400 mt-1">
                  Supplier not in list? <a href="/suppliers" className="text-emerald-600 hover:underline">Add new supplier</a>
                </div>
              </div>
            ...
            <div className="table-container mb-6 overflow-x-auto">
              ...
              <div className="mt-4 flex justify-end gap-8 px-4 text-sm font-medium text-slate-600">
                  ...
                  <div className="text-emerald-700 font-bold text-lg">Total: ₹ {grandTotal.toFixed(2)}</div>
              </div>
            </div>

            <div className="flex gap-4 pt-4 border-t border-slate-100">
              <button onClick={addRow} className="btn px-4 py-2 border border-emerald-200 text-emerald-600 rounded-lg hover:bg-emerald-50 flex items-center gap-2"><FiPlus /> Add Row</button>
              <button onClick={handleSubmit} className="btn ml-auto px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 shadow flex items-center gap-2"><FiSave /> Save Purchase</button>
            </div>
          </div>
      ) : (
          <div>
                          ...
                          filteredPurchases.map(p => (
                             <tr key={p.id} className="hover:bg-slate-50/50">
                                 ...
                                 <td className="p-4 text-center">
                                  ...
                                  <td className="p-4 text-center">
                                      <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase ${p.status === 'CANCELLED' ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>{p.status}</span>
                                  </td>
      ...
      {/* Details Modal */}
      {selectedPurchase && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedPurchase(null)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="p-6 bg-slate-50 border-t flex justify-between items-center">
                      ...
                      <div className="text-xl font-bold text-slate-800">
                          ...
                          Total: <span className="text-emerald-600">₹{parseFloat(selectedPurchase.totalAmount).toFixed(2)}</span>
                      </div>
                  </div>
              </div>
          </div>
