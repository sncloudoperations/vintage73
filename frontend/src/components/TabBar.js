...
export default function TabBar() {
  ...
  return (
    <div className="bg-transparent px-4 pt-2 flex items-center gap-2 relative">
      {/* Scrollable tabs container */}
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar flex-1" style={{ paddingRight: tabs.length > 1 ? '120px' : '0' }}>
        {tabs.map((tab) => (
          <div
            key={tab.path}
      ...
      {tabs.length > 1 && (
        <button
          ...
          className="absolute right-6 top-2 text-xs text-slate-400 hover:text-red-500 px-3 py-1 whitespace-nowrap z-20 border-l border-gray-200 pl-4 bg-white/95 backdrop-blur-sm"
        >
          Clear All
        </button>
      )}
