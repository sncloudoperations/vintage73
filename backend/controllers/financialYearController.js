...
exports.createFinancialYear = async (req, res) => {
    try {
        ...
        if (existing) {
            return res.status(400).json({ error: 'Date range overlaps with an existing Financial Year.' });
        }
...
exports.previewClosing = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.closeFinancialYear = async (req, res) => {
    try {
        ...
        const year = await prisma.financialYear.findUnique({ where: { id: parseInt(id) } });
        if (!year) return res.status(404).json({ error: 'Financial Year not found' });
        if (year.isClosed) return res.status(400).json({ error: 'Year is already closed.' });
        ...
        if (journalEntries.length === 0) {
            return res.status(400).json({ error: 'No transactions to close.' });
        }
